...
def topoplot_with_colorbar(data, raw_info, vmin=None, vmax=None, cmap='bone', mask=None):
    ...
    :param data: data to plot, should be size (channels,)
    ...
    :param mask: mask corresponding the data, will be plotted in black 'x', default = None
    """
    import mne
...
def plot_with_sem_one_line(x, y, ax, ax_num, xlim, ylim, color_y, color_y_sem,
                           label_y=None, marker_y=None, alpha_level=1,
...
def plot_with_sem_color(x, y, ax, ax_num, xlim, color_y, color_y_sem, alpha_level, label_y,
                        xlabel='Time, s', ylabel='Amplitude, V'):
    """
    ...
    if ax_num:
        for yi, yval in enumerate(y):
            ...
            ax[ax_num].plot(x, np.mean(yval, axis=0), color=color_y[yi], label=label_y[yi])
        ...
        for yi, yval in enumerate(y):
            ax.fill_between(x, np.mean(yval, axis=0) - sem(yval, axis=0),
                            np.mean(yval, axis=0) + sem(yval, axis=0),
                            color=color_y_sem[yi], alpha=alpha_level[yi])
            ax.plot(x, np.mean(yval, axis=0), color=color_y[yi], label=label_y[yi])
...
def parula_map_backward():
    ...
    :return:
    parula_back_map : instance of matplotlib.colors.LinearSegmentedColormap
    """
    from matplotlib.colors import LinearSegmentedColormap
    ...
    parula_back_map = LinearSegmentedColormap.from_list('parula', cm_data)
    return parula_back_map


def plot_brain_views(data, clim, filename, cmap='bone'):
    """
    Plots 4 brain views of the chosen data and store it to .png files.

    :param numpy.ndarray data: data to plot, should be the size of source space (dimensions - 1D)
    :param dict clim: color limits
    :param str filename: the beginning of the file name
    :param cmap: either str or instance of LinearSegmentedColormap, colormap to use, default = 'bone'
    """
    ...
    tmp.save_image(filename + '_rh_lat.png')
    tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=cmap,
                               transparent=False, views='medial')
    tmp.save_image(filename + '_rh_med.png')
    tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=cmap,
                               transparent=False, views='lateral')
    tmp.save_image(filename + '_lh_lat.png')
    tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=cmap,
                               transparent=False, views='medial')
    tmp.save_image(filename + '_lh_med.png')
