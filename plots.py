...
from tools_general import list_from_many, load_json_to_numpy, load_pickle, load_json, scaler_transform, save_pickle
from tools_lifedataset import read_medications, composite_attention, composite_memory, composite_executive, read_age, \
    read_gender
from tools_plotting import plot_with_sem_one_line, topoplot_with_colorbar, parula_map, plot_with_sem_color, \
    annotated_heatmap, parula_map_backward
from tools_signal import pk_latencies_amplitudes, apply_spatial_filter, lda_
mpl.use("Qt5Agg")
import matplotlib.pyplot as plt
import numpy as np
from scipy.stats import sem, pearsonr, spearmanr, ttest_rel, t
...
for i_subj, subj in enumerate(ids):
    avg_erp_pk = pk_latencies_amplitudes(avg_erp_t[i_subj, pz_idx], np.array([0.2, 1]), erp_times, direction='pos')[0][1]
    if avg_erp_pk != 0:
...
cluster_stats = spatio_temporal_cluster_test(X, n_permutations=1000,
                                             threshold=t_threshold, stat_fun=mne.stats.ttest_ind_no_p, tail=0,
...
for i in range(len(time_inds)):
    F_obs_sig[time_inds[i], space_inds[i]] = 1
...
#     else:
#         data_to_plot[i] = 1000
stc_plot._data = data_to_plot
clim = dict(kind='value', lims=[-1 * np.nanmax(np.abs(stc_plot._data)), 0, 1 * np.nanmax(np.abs(stc_plot._data))])
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('corr_rh_lat.png')
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('corr_rh_med.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('corr_lh_lat.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('corr_lh_med.png')
...
win_samples = np.array([np.argmin(np.abs(erp_times_dec - win[0])), np.argmin(np.abs(erp_times_dec - win[1]))])
X1 = np.transpose(np.mean(stc_p300[:, :, win_samples[0]:win_samples[1]] ** 2, axis=2).reshape((len(ids), n_source, 1)),
                  [0, 2, 1])
X2 = np.transpose(np.mean(stc_nop300[:, :, win_samples[0]:win_samples[1]] ** 2, axis=2).reshape((len(ids), n_source, 1)),
                  [0, 2, 1])
...
# ENV
X1 = np.transpose(np.mean(stc_nop300_alpha_env[:, :, win_samples[0]:win_samples[1]], axis=2).reshape(
    (len(ids), n_source, 1)),[0, 2, 1]) / \
     np.transpose(np.mean(stc_p300_alpha_env[:, :, win_samples[0]:win_samples[1]], axis=2).reshape(
    (len(ids), n_source, 1)),
    [0, 2, 1])
X2 = np.ones(X1.shape)
...
# ----------------------------------------------
stc_plot = stc_fixed.copy().crop(tmin=0, tmax=0)
stc_plot._data = np.multiply(X_avg_diff, np.array(t_vox) > thr_source)
clim = dict(kind='value', lims=[np.min(X_avg_diff), np.mean(X_avg_diff), np.max(X_avg_diff)])
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('p300_rh_lat.png')
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('p300_rh_med.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('p300_lh_lat.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('p300_lh_med.png')
...
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map_backward(),
                           transparent=False, views='lateral')
tmp.save_image('alpha_env_rh_lat.png')
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map_backward(),
                           transparent=False, views='medial')
tmp.save_image('alpha_env_rh_med.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map_backward(),
                           transparent=False, views='lateral')
tmp.save_image('alpha_env_lh_lat.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map_backward(),
                           transparent=False, views='medial')
tmp.save_image('alpha_env_lh_med.png')

# plot cluster mask
cluster_bool = np.multiply(cluster_erp_bool, cluster_env_bool)
stc_plot = stc_fixed.copy().crop(tmin=0, tmax=0)
stc_plot._data = cluster_bool
clim = dict(kind='value', lims=[0, np.mean(stc_plot._data), np.max(stc_plot._data)])
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('cluster_rh_lat.png')
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('cluster_rh_med.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('cluster_lh_lat.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('cluster_lh_med.png')
...
stc_plot = stc_fixed.copy().crop(tmin=0, tmax=0)
stc_plot._data = stc_bsi_avg
clim = dict(kind='value', lims=[-1 * np.nanmax(np.abs(stc_plot._data)), 0, 1 * np.nanmax(np.abs(stc_plot._data))])
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('bsi_rh_lat.png')
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('bsi_rh_med.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('bsi_lh_lat.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('bsi_lh_med.png')
...
stc_plot = stc_fixed.copy().crop(tmin=0, tmax=0)
stc_plot._data = bsi_corr
clim = dict(kind='value', lims=[np.min(stc_plot._data), np.mean(stc_plot._data), np.max(stc_plot._data)])
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('bsi_corr_full_rh_lat.png')
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('bsi_corr_full_rh_med.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('bsi_corr_full_lh_lat.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('bsi_corr_full_lh_med.png')
...
ids_attention_executive_ids, idx1, idx2 = np.intersect1d(executive_ids, attention_ids, return_indices=True)
ids_attention_executive_memory_ids, idx3, idx4 = np.intersect1d(ids_attention_executive_ids, memory_ids,
                                                                return_indices=True)
...
for i, var_i in enumerate(var_to_corr):
    for j, var_j in enumerate(var_to_corr):
        if i > j:
            ...
            try:
                corr_tmp_par = partial_corr(data_totest, var_i, var_j, covar='age', method='spearman')
                corr_par[i, j] = corr_par[j, i] = corr_tmp_par['r']
                corr_par_pval[i, j] = corr_par_pval[j, i] = corr_tmp_par['p-val']
            except:
                print('no age')

annotated_heatmap(var_to_corr[:4], var_to_corr[5:], np.round(corr_par[:4, 5:], 2).T, cbarlabel='Spearman coefficient',
                  cmap=parula_map())
annotated_heatmap(var_to_corr[:4], var_to_corr[5:], 1 * (corr_par_pval < (0.05 / 3))[:4, 5:].T,
                  cbarlabel='Spearman coefficient',
                  cmap=parula_map())
annotated_heatmap(var_to_corr[:4], var_to_corr[5:], np.round(corr_par_pval[:4, 5:], 4).T,
                  cbarlabel='Spearman coefficient',
                  cmap=parula_map())
...
stc_plot = stc_fixed.copy().crop(tmin=0, tmax=0)
stc_plot._data = bsi_mode
clim = dict(kind='value', lims=[-1 * np.nanmax(np.abs(stc_plot._data)), 0, 1 * np.nanmax(np.abs(stc_plot._data))])
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('tmp_rh_lat.png')
tmp = stc_plot.copy().plot(hemi='rh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('tmp_rh_med.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='lateral')
tmp.save_image('tmp_lh_lat.png')
tmp = stc_plot.copy().plot(hemi='lh', size=(500, 500), clim=clim, colormap=parula_map(),
                           transparent=False, views='medial')
tmp.save_image('tmp_lh_med.png')
